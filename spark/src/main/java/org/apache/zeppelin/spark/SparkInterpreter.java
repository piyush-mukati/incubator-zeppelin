...
public class SparkInterpreter extends Interpreter {
  ...
  private  String sparkReplClassPath;
  ...
  public SparkContext createSparkContext() {
    ...
    if (classServerUri == null) {
      try { // for spark 1.3x
        Method classServer =
          initialSparkInterpreterContext.getInterpreter().intp().getClass().getMethod("classServerUri");
  ...
  public List<String> completion(String buf, int cursor, InterpreterContext context) {
    logger.debug("method  Completion pid=" + context.getParagraphId() + "  nid= " + context.getNoteId());
    ...
    logger.debug("  Completion pid=" + context.getParagraphId() + "  nid= " + context.getNoteId() + "returning :" + ret
  ...
  CacheLoader<String, SparkInterpreterContext> SparkInterpreterContextLoader =
    new CacheLoader<String, SparkInterpreterContext>() {
      public SparkInterpreterContext load(String key) {
        return createSparkInterpreterContext(key);
      }
    };

  RemovalListener<String, SparkInterpreterContext> SparkInterpreterContextRemovalListener =
    new RemovalListener<String, SparkInterpreterContext>() {
      @Override
  ...
  public InterpreterResult interpret(String line, InterpreterContext context) {
    try {
      ...
      logger.info("NoteId:" + context.getNoteId() + " ParagraphId: " + context.getParagraphId()
        + " unable to setInterpreterContext, failed to get SparkInterpreterContext due to : " + e.getCause(), e);
  ...
  public InterpreterResult interpret(String[] lines, InterpreterContext context) {
    ...
    try {
      ...
      logger.info("NoteId:" + context.getNoteId() + " ParagraphId: " + context.getParagraphId()
        + " unable to setGui, failed to get SparkInterpreterContext due to : " + e.getCause(), e);
    }
  ...
  public InterpreterResult interpretInput(String[] lines, InterpreterContext context) throws ExecutionException {
    SparkEnv.set(env);
    ...
    if (r == Code.INCOMPLETE) {
      ...
      if (out.size() > outputStreamMaxBytes) {
        String truncatedOut = out.toString().substring(0, outputStreamMaxBytes)
  ...
  private void getConfigs(Properties properties) {
    try {
      ...
      logger
        .error("error while getting value of " + ZEPPELIN_SPARK_CONCURRENT_SESSIONS + " setting default value of 10",
          e);
      m_maxThreads = 10;
    }
    try {
      sparkInterpreterContexTTLHours =
        Integer.parseInt(properties.getProperty(ZEPPELIN_SPARK_INTERPRETER_CONTEXT_TTL_HOURS, "24"));
      logger
        .info("got value for " + ZEPPELIN_SPARK_INTERPRETER_CONTEXT_TTL_HOURS + " : " + sparkInterpreterContexTTLHours);
    } catch (Exception e) {
      logger.error(
        "error while getting value of " + ZEPPELIN_SPARK_INTERPRETER_CONTEXT_TTL_HOURS + " setting default value of 24",
        e);
